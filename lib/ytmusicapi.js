export class YTMusic {
  ...
  async getSong(videoId) {
    try {
      const params = { videoId, params: this._encodeVideoParams() };
  ...
  async getAlbum(browseId) {
    try {
      const params = { browseId, context: this.context };
  ...
  async getPlaylist(playlistId, limit = 100) {
    try {
      const params = { playlistId, params: this._encodePlaylistParams(limit) };
      const response = await this._makeRequest('browse', params);
  ...
  async getCharts(country = null) {
    try {
      const params = {
        ...
        context: { ...this.context, client: { ...this.context.client, gl: country || 'US' } }
      };
      const response = await this._makeRequest('browse', params);
  ...
  async getMoodCategories() {
    try {
      const params = { browseId: 'FEmusic_moods_and_genres', context: this.context };
      const response = await this._makeRequest('browse', params);
  ...
  async getMoodPlaylists(categoryId) {
    try {
      const params = { browseId: categoryId, context: this.context };
      const response = await this._makeRequest('browse', params);
  ...
  async getWatchPlaylist(videoId = null, playlistId = null, radio = false, shuffle = false, limit = 25) {
    try {
      const params = { videoId, playlistId, radio, shuffle, params: this._encodeWatchPlaylistParams(limit) };
  ...
  async _makeRequest(endpoint, params) {
    ...
    const requestBody = params.continuation ? {
  ...
  _encodeSearchParams(query, filter = null, continuationToken = null, ignoreSpelling = false) {
    if (continuationToken) return { continuation: continuationToken };
    const searchParams = { query, params: this._getFilterParams(filter) };
    if (ignoreSpelling) searchParams.params += '&ignore_spelling=true';
    return searchParams;
  }
  ...
  _getFilterParams(filter) {
    const filterMap = {
      ...
      'channels': 'EgWKAQIgAWoKEAMQBBAJEAoQBQ%3D%3D'
    };
    return filterMap[filter] || 'Eg-KAQwIARAAGAAgACgAMABqChAEEAUQAxAKEAk%3D';
  }

  _encodeVideoParams() {
    return btoa(JSON.stringify({ videoId: '', context: this.context }));
  }

  _encodePlaylistParams(limit) {
    return btoa(JSON.stringify({ browseId: '', params: `6gPTAUNwc0JBRUNBQ0F%3D&limit=${limit}` }));
  }

  _encodeWatchPlaylistParams(limit) {
    return btoa(JSON.stringify({ videoId: '', playlistId: '', radio: false, shuffle: false, params: `6gPTAUNwc0JBRUNBQ0F%3D&limit=${limit}` }));
  }
      ...
      for (const key of Object.keys(cc)) {
        ...
        for (const entry of contents) {
          if (entry.musicShelfRenderer) {
            ...
            continuationToken = cont?.nextContinuationData?.continuation || cont?.reloadContinuationData?.continuation || continuationToken;
  ...
  _parseSuggestions(data) {
    ...
    allContents.forEach(content => {
      ...
      let suggestionText = runs.map(r => r.text || '').join('');
    ...
    contents.forEach(section => {
      if (section.musicShelfRenderer) {
        charts.push({
          ...
          items: (section.musicShelfRenderer.contents || []).map(item => this._parseMusicItem(item.musicResponsiveListItemRenderer)).filter(Boolean)
        });
      }
    });
    return charts;
  ...
  _parseMoodCategories(data) {
    ...
    contents.forEach(section => {
      if (section.musicCarouselShelfRenderer) {
        categories.push({
          title: section.musicCarouselShelfRenderer.header?.musicCarouselShelfBasicHeaderRenderer?.title?.runs?.[0]?.text,
          items: (section.musicCarouselShelfRenderer.contents || []).map(item => this._parseMusicItem(item.musicTwoRowItemRenderer)).filter(Boolean)
        });
      }
    });
    return categories;
  ...
  _parseMoodPlaylists(data) {
    ...
    contents.forEach(section => {
      if (section.musicShelfRenderer) {
        playlists.push(...(section.musicShelfRenderer.contents || []).map(item => this._parseMusicItem(item.musicResponsiveListItemRenderer)).filter(Boolean));
      }
    });
    return playlists;
  }
  ...
  _parseWatchPlaylist(data) {
    ...
    const tracks = contents.filter(item => item.playlistPanelVideoRenderer).map(item => {
    ...
    return { tracks, playlistId: data?.watchNextRenderer?.playlistId };
  }
  ...
  _parseMusicItem(item) {
    ...
    if (!watchPlaylistId && item.menu?.menuRenderer?.items) {
      const mixItem = item.menu.menuRenderer.items.find(i => i.menuNavigationItemRenderer?.icon?.iconType === 'MIX');
      if (mixItem) watchPlaylistId = mixItem.menuNavigationItemRenderer.navigationEndpoint?.watchEndpoint?.playlistId;
    }

    const browseId = item.navigationEndpoint?.browseEndpoint?.browseId;
    const playlistId = item.navigationEndpoint?.browseEndpoint?.browseEndpointContextSupportedConfigs?.browseEndpointContextMusicConfig?.pageType === 'MUSIC_PAGE_TYPE_PLAYLIST' ? item.navigationEndpoint?.browseEndpoint?.browseId : null;
    ...
    if (!duration) {
      for (const run of subtitle) {
        ...
        const match = text.match(/\b(?:(\d{1,2}):)?(\d{1,2}):(\d{2})\b/) || text.match(/\b(\d{1,2}):(\d{2})\b/);
        if (match) {
          duration = text;
          break;
        }
    ...
    const artists = [];
    let album = null, views = null, year = null;
    subtitle.forEach((run, index) => {
      if (run.navigationEndpoint?.browseEndpoint) {
        const pageType = run.navigationEndpoint.browseEndpoint.browseEndpointContextSupportedConfigs?.browseEndpointContextMusicConfig?.pageType;
        if (pageType === 'MUSIC_PAGE_TYPE_ARTIST' || pageType === 'MUSIC_PAGE_TYPE_USER_CHANNEL') artists.push({ name: run.text, id: run.navigationEndpoint.browseEndpoint.browseId });
        else if (pageType === 'MUSIC_PAGE_TYPE_ALBUM') album = { name: run.text, id: run.navigationEndpoint.browseEndpoint.browseId };
      } else if (run.text) {
        const text = run.text.trim();
        if (!text || text === '•' || text === '&') return;
        if (text.includes('views') || text.includes('plays')) views = text;
        else if (/^\d{4}$/.test(text)) year = text;
        else if (index === 0 || artists.length === 0) artists.push({ name: text, id: null });
      }
    });

    const resultType = playlistId ? 'playlist' : browseId?.startsWith('UC') ? 'artist' : browseId?.startsWith('MPREb_') ? 'album' : 'song';
    const result = {
      category: resultType.charAt(0).toUpperCase() + resultType.slice(1) + 's',
      resultType,
      title,
      thumbnails: (item.thumbnail?.musicThumbnailRenderer?.thumbnail?.thumbnails || []).map(t => ({ url: t.url, width: t.width, height: t.height })),
      isExplicit: item.badges?.some(b => b.musicInlineBadgeRenderer?.icon?.iconType === 'MUSIC_EXPLICIT_BADGE') || false
    };

    if (watchPlaylistId) result.playlistId = watchPlaylistId;
    if (videoId) {
      ...
      if (section.musicShelfRenderer) {
        (section.musicShelfRenderer.contents || []).forEach(item => {
          if (item.musicResponsiveListItemRenderer) {
            const parsed = this._parseMusicItem(item.musicResponsiveListItemRenderer);
  ...
  _parseAlbumsFromContents(contents) {
    return this._parseItemsFromContents(contents, 'Albums');
  }

  _parseSinglesFromContents(contents) {
    return this._parseItemsFromContents(contents, 'Singles');
  }

  _parseItemsFromContents(contents, title) {
    const section = contents.find(s => (s.musicCarouselShelfRenderer?.header?.musicCarouselShelfBasicHeaderRenderer?.title?.runs?.[0]?.text === title));
    if (!section) return [];
    return (section.musicCarouselShelfRenderer.contents || []).map(item => this._parseMusicItem(item.musicTwoRowItemRenderer)).filter(Boolean);
  }
}
