...
class JioSaavn {
  ...
  _findStrictMatchingTrack(processedResults, title, requestedArtists, debug) {
    ...
    for (const track of processedResults) {
      ...
      // Relaxed check: verify that every requested artist is found in the track's primary, featured, or all artists list
      const trackArtistsAll = new Set([
        ...(track.artists?.primary || []).map(a => normalizeString(a.name)),
        ...(track.artists?.featured || []).map(a => normalizeString(a.name)),
        ...(track.artists?.all || []).map(a => normalizeString(a.name))
      ]);

      const allFound = [...normalizedRequestedArtists].every(reqArtist =>
        [...trackArtistsAll].some(trackArtist => trackArtist.includes(reqArtist) || reqArtist.includes(trackArtist))
      );
